...
pub const SignalKind = switch (builtin.os.tag) {
    .windows => enum(u8) {
        ...
        interrupt = @intFromEnum(std.posix.SIG.INT),
        terminate = @intFromEnum(std.posix.SIG.TERM),
        hangup = @intFromEnum(std.posix.SIG.HUP),
        alarm = @intFromEnum(std.posix.SIG.ALRM),
        child = @intFromEnum(std.posix.SIG.CHLD),
        pipe = @intFromEnum(std.posix.SIG.PIPE),
        quit = @intFromEnum(std.posix.SIG.QUIT),
        user1 = @intFromEnum(std.posix.SIG.USR1),
        user2 = @intFromEnum(std.posix.SIG.USR2),
        _,
    },
};
