...
pub fn spawnTask(
    ...
    // New tasks are homed round-robin and delivered to the home executor's
    // overflow queue with a wake of its loop. Initial spread matters: on epoll
    // and kqueue backends a socket is pinned to the loop that registers it, so
    // a task's first executor decides where its I/O lives.
