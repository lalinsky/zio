...
/// Application code should use higher-level abstractions from the sync module instead.
///
/// This is a space-optimized variant of WaitQueue that uses only 8 bytes by storing
/// the tail pointer in the head node's userdata field. This is useful for implementing
/// standard library interfaces that only provide 64 bits of state (e.g., std.Thread.Mutex).
///
/// Uses the same tagged pointer scheme as WaitQueue:
/// - 0b00: Sentinel state 0
/// - 0b01: Sentinel state 1
/// - ptr (>1): Pointer to head of wait queue
