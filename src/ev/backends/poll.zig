...
fn getEvents(completion: *Completion) @FieldType(net.pollfd, "events") {
    return switch (completion.op) {
        ...
        .pipe_read, .file_read_streaming => if (builtin.os.tag == .windows) unreachable else net.POLL.IN,
        .pipe_write, .file_write_streaming => if (builtin.os.tag == .windows) unreachable else net.POLL.OUT,
...
fn getPollType(op: Op) PollEntryType {
    return switch (op) {
        ...
        .pipe_read, .pipe_write, .file_read_streaming, .file_write_streaming => if (builtin.os.tag == .windows) unreachable else .send_or_recv,
...
fn getHandle(completion: *Completion) NetHandle {
    return switch (completion.op) {
        ...
        inline .pipe_read, .pipe_write, .file_read_streaming, .file_write_streaming => |op| if (builtin.os.tag == .windows) unreachable else completion.cast(op.toType()).handle,
...
pub fn checkCompletion(c: *Completion, item: *const net.pollfd) CheckResult {
    switch (c.op) {
        ...
        // Pipe operations not supported on Windows (poll uses SOCKET, not HANDLE - Windows uses IOCP)
        inline .pipe_read, .file_read_streaming => |op| if (builtin.os.tag == .windows) unreachable else {
        ...
        inline .pipe_write, .file_write_streaming => |op| if (builtin.os.tag == .windows) unreachable else {
            const data = c.cast(op.toType());
