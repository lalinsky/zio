...
pub fn createPipe(rt: *Runtime) (os.fs.PipeError || Cancelable)!PipePair {
    std.debug.print("[FS] createPipe: creating op\n", .{});
    var op = ev.PipeCreate.init();
    std.debug.print("[FS] createPipe: calling waitForIo\n", .{});

    waitForIo(rt, &op.c) catch |err| {
        std.debug.print("[FS] createPipe: waitForIo failed with error: {}\n", .{err});
        std.debug.print("[FS] createPipe: completion.err = {?}\n", .{op.c.err});
        std.debug.print("[FS] createPipe: completion.state = {}\n", .{op.c.state});
        return err;
    };

    std.debug.print("[FS] createPipe: waitForIo returned\n", .{});
    const fds = try op.getResult();
    std.debug.print("[FS] createPipe: getResult returned fds\n", .{});
    return .{
...
pub const File = struct {
    ...
    pub fn stdin() File {
        return .{ .fd = os.fs.stdin() };
    }

    pub fn stdout() File {
        return .{ .fd = os.fs.stdout() };
    }

    pub fn stderr() File {
        return .{ .fd = os.fs.stderr() };
    }
...
test "Pipe: basic read and write" {
    ...
    std.debug.print("Creating pipe\n", .{});
    const pipe = try createPipe(rt);
    std.debug.print("Got pipe\n", .{});
    defer pipe.close(rt);
...
test "Pipe: vectored I/O" {
    ...
    // Write using WriteBuf
    const slices = [_][]const u8{ "Hello, ", "world!" };
    var storage: [2]os.iovec_const = undefined;
    const write_buf = ev.WriteBuf.fromSlices(&slices, &storage);

    const bytes_written = try pipe.write.writeBuf(rt, write_buf, .none);
    try std.testing.expectEqual(13, bytes_written);
